...
pca_train <- prcomp(data_train[,-1], scale=TRUE, rank=7)

var_explained <- pca_train$sdev^2 /sum(pca_train$sdev^2)
cum_var_explained <- NULL
for(i in seq(length(var_explained))){
	cum_var_explained <- c(cum_var_explained, sum(var_explained[1:i]))
	}

plot(c(1:30), cum_var_explained, type='b', xlab='Number of PCA Components', ylab='Variance Explained')

pca_test <- data.frame(predict(pca_train, data_test[,-1]))
